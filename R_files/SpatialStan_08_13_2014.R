...
fit.compile <- stan(file = "SpatialStan_08_13_2014.stan", data = mvn_prec_data, chains = 0)
fit.test <- stan(fit = fit.compile, data = mvn_prec_data, chains = 1, iter = 30, refresh = 5)

chain10 <- stan(fit = fit.compile, data = mvn_prec_data, chains = 1, chain_id = 10, iter = 2500)
chain11 <- stan(fit = fit.compile, data = mvn_prec_data, chains = 1, chain_id = 11, iter = 2500)
chain12 <- stan(fit = fit.compile, data = mvn_prec_data, chains = 1, chain_id = 12, iter = 2500)
...
for(i in c(1:12)){
  file <- paste0("Stan_tests/Aug_13th/Aug_13th_chain",i,".RData")  
  load(file)
}

library(rstan)
stanfit <- sflist2stanfit(list(chain1,chain2,chain3,chain4,chain5,chain6,
                               chain7,chain8,chain9,chain10,chain11,chain12))

monitor(stanfit, digits = 3)

stanfit <- sflist2stanfit(list(chain1,chain3,chain4,chain5,chain6,
                               chain7,chain8,chain9,chain10,chain11,chain12))
