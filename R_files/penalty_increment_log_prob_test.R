...
for(j in 1:4){
increment_log_prob(0.5 * R * log(sigma_sq[j]) + 0.5 * log(determinant(D - rho[j] * KK)));
}
...
library(rstan)
fit.compile <- stan(model_code = stan_code, data = data.list, chains = 0)
fit.compile <- stan(file = "penalty_increment_log_prob_test.stan", data = data.list, chains = 0)
fit.test1 <- stan(fit = fit.compile, data = data.list, chains = 1, iter = 50)
fit.test <- stan(fit = fit.compile, data = data.list, chains = 1, iter = 1000)