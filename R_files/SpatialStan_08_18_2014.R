...
X[1,,] <- X_urb
X[2,,] <- X_union
X[3,,] <- M # for the intercept/region-period effects
...
Z <- cbind(Data$dem, Data$laborcomm, aaprop)
...
rm(list=setdiff(ls(), "mvn_prec_data"))

library(rstan)
set_cppo("fast")


fit.compile <- stan(file = "SpatialStan_08_18_2014.stan", data = mvn_prec_data, chains = 0)
...
for(j in 11:12){
  stan_out <- stan(fit = fit.compile, data = mvn_prec_data, chains = 1, chain_id = j, iter = 2500)
  chn <- paste0("chain_",j)
  assign(chn, stan_out)
}

save(chain_11, file = "Aug18th_chain_11.RData")
save(chain_12, file = "Aug18th_chain_12.RData")




for(i in c(1:12)){
  file <- paste0("Stan_tests/Aug18th_chain_",i,".RData")  
  load(file)
}

library(rstan)
stanfit <- sflist2stanfit(list(chain_1,chain_2,chain_3,chain_4,chain_5,chain_6,
                               chain_7,chain_8,chain_9,chain_10,chain_11,chain_12))
...
library(lattice)
# stanfit S3 method for parallelplot() function in lattice package
parallelplot.stanfit <- function(x, ...) {
  chain <- factor(paste("chain", rep(1:ncol(x), each = nrow(x))),
                  levels = paste("chain", 1:ncol(x)), ordered = FALSE)
  parallelplot(~as.matrix(x) | chain, horizontal.axis = FALSE, ...)
}

parallelplot(stanfit, scales = list(x = list(alternating = 0)), layout = c(3,4))
